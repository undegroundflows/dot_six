...
function ENT:Initialize()
	...
	self:SetModel("models/6000/body.mdl")
	self.BaseClass.Initialize(self)
	self:SetPos(self:GetPos() + Vector(0,0,140))
	
	-- Create seat entities
	self.DriverSeat = self:CreateSeat("driver",Vector(430,-14.5,-16))
	...
	-- Create bogeys
	self.FrontBogey = self:CreateBogey(Vector( 325-5,0,-85),Angle(0,180,0),true)
	self.RearBogey  = self:CreateBogey(Vector(-325+0,0,-85),Angle(0,0,0),false)
...
function ENT:Think()
	...
	if (CurTime() - self.TextureTime) > 1.0 then
		...
		if self.SignsList then
			for k,v in pairs(self:SignsList()) do
